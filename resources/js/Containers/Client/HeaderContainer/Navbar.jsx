    ...
    return (
        <Link href={URL} >
            <div className='flex flex-row items-center'>
                <ApplicationLogo className="ltr:mr-3 rtl:ml-3 h-8 md:h-12" />
...
NavBar.List = function NavBarList({ children }) {
    return (
        <ul className='hidden lg:flex items-center gap-6 lg:gap-x-12 capitalize'>
            {children}
        </ul>
    )
}
...
NavBar.Item = function NavBarItem({ URL, children }) {
    return (
        <li className='font-semibold text-sm lg:text-base transition-all duration-150 text-accent hover:text-primary border-primary cursor-pointer'>
            <Link href={URL} >
                {children}
            </Link>
        </li>
...
NavBar.Label = function NavBarLabel({ children }) {
    return (
...
NavBar.SelectLang = function NavBarSelectLang({ locale }) {
    return (
        <Dropdown className='hidden lg:block rounded-full transition-all'>
            <Dropdown.Trigger>
                <button className="flex gap-1 items-center text-base text-accent px-3 py-1 hover:text-primary focus:outline-none transition-all ease-in-out duration-150">
                    <p>{locale == "en" ? "English" : "العربية"}</p>
                    <MdKeyboardArrowDown />
                </button>
            </Dropdown.Trigger>

            <Dropdown.Content className="bg-minorBackground mt-5 w-40 text-accent text-right ltr:right-0 rtl:text-left rtl:left-0">
                <Dropdown.Atag href={route('locale', 'ar')} className="hover:bg-majorBackground border-t-2 border-accent">
                    العربية
                </Dropdown.Atag>
                <Dropdown.Atag href={route('locale', 'en')} className="hover:bg-majorBackground rounded-b-md">
                    English
                </Dropdown.Atag>
            </Dropdown.Content>
        </Dropdown>
...
NavBar.Hamburger = function NavBarHamburger({ isSlideNavOpened, setIsSlideNavOpened }) {
    ...
    return (
        <button className='lg:hidden w-6 h-5 flex flex-col justify-between cursor-pointer'
